...
SELECT 
  ...
  CASE 
    ...
    WHEN s.mobile_number = au.phone THEN '✅ Match by phone'
    ELSE '⚠️ Partial match'
  END as match_type
FROM shops s
LEFT JOIN auth.users au ON (
  s.email = au.email 
  OR s.mobile_number = au.phone
  OR s.owner_phone = au.phone
)
WHERE s.user_id IS NULL 
   OR NOT EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = s.user_id)
ORDER BY s.created_at DESC;
...
-- Matching logic: email, mobile_number, or owner_phone
-- ========================================
...
SELECT 
  ...
  CASE 
    ...
    WHEN s.owner_phone = au.phone THEN 'Match by phone (owner_phone)'
    ELSE 'No match found'
  END as match_method
FROM shops s
LEFT JOIN auth.users au ON (
  ...
  OR (s.owner_phone IS NOT NULL AND s.owner_phone = au.phone)
)
WHERE s.user_id IS NULL 
   OR NOT EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = s.user_id)
ORDER BY s.created_at DESC;
...
-- Step 9.4: Fix shops by matching owner_phone
UPDATE shops s
SET user_id = au.id
FROM auth.users au
WHERE s.owner_phone IS NOT NULL 
  AND s.owner_phone = au.phone
  AND (s.user_id IS NULL OR NOT EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = s.user_id));

SELECT 
  '✅ STEP 9.4: Fixed shops by owner_phone match' as step,
  COUNT(*) as shops_fixed
FROM shops s
WHERE s.user_id IS NOT NULL
  AND EXISTS (SELECT 1 FROM auth.users au WHERE au.phone = s.owner_phone AND au.id = s.user_id);
