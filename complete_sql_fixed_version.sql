...
-- INCLUDES DELIVERY AGENTS TABLE WITH CORRECT FOREIGN KEY
-- ============================================================
-- This script:
-- 0. ✅✅✅ IMMEDIATE FIX: Fixes delivery_agents foreign key constraint (RUNS FIRST!)
...
-- 10. ✅✅✅ CORRECTLY configures foreign key to point to auth.users(id) - NOT users table!
...
-- ========================================
-- PART 0: IMMEDIATE FIX FOR DELIVERY AGENTS FOREIGN KEY (RUNS FIRST!)
-- ========================================
-- This MUST run first to fix the foreign key constraint issue
-- This fixes: "Key (user_id)=(...) is not present in table 'users'"
-- This comprehensive fix drops ALL constraints and adds the correct one
-- ========================================

-- Step 0.1: Drop ALL foreign key constraints on delivery_agents.user_id (COMPREHENSIVE)
DO $$
DECLARE
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Also try dropping the common constraint name directly (using EXECUTE to avoid syntax issues)
    BEGIN
      ...
      END IF;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'ℹ️ Constraint delivery_agents_user_id_fkey does not exist or already dropped';
    END;
    ...
    END IF;
  ELSE
    RAISE NOTICE 'ℹ️ delivery_agents table does not exist yet - will be created with correct constraint in Part 12';
  END IF;
END $$;

-- Step 0.2: Clean up corrupted data (if table exists)
DO $$
DECLARE
...
BEGIN
  IF EXISTS (
    ...
    IF deleted_count > 0 THEN
      RAISE NOTICE '✅ Cleaned up % row(s) with invalid user_id', deleted_count;
    ELSE
...
-- Step 0.3: Add the CORRECT constraint pointing to auth.users (if table exists)
-- This is the CRITICAL fix - must point to auth.users, NOT users
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    ...
    -- Drop constraint if it exists (already done in Step 0.1, but ensure it's dropped)
    BEGIN
      EXECUTE 'ALTER TABLE delivery_agents DROP CONSTRAINT IF EXISTS delivery_agents_user_id_fkey CASCADE';
    EXCEPTION WHEN OTHERS THEN
      NULL; -- Ignore errors
    ...
    BEGIN
      ...
      RAISE NOTICE '✅✅✅ Added correct foreign key constraint pointing to auth.users';
    EXCEPTION WHEN OTHERS THEN
...
-- Step 0.4: Verify the fix (if table exists)
SELECT 
...
-- IMPORTANT: Foreign key correctly points to auth.users(id), NOT users table
-- ========================================

-- Step 12.0: COMPREHENSIVE FIX - Drop ALL existing incorrect foreign key constraints
-- This MUST run first to fix any existing wrong constraints
DO $$
DECLARE
  constraint_record RECORD;
  dropped_count INTEGER := 0;
BEGIN
  IF EXISTS (
    ...
    END IF;
  ELSE
    RAISE NOTICE 'ℹ️ delivery_agents table does not exist yet - will be created with correct constraint';
  END IF;
END $$;

-- Step 12.1: Clean up any corrupted data (delete rows with invalid user_id)
DO $$
DECLARE
  deleted_count INTEGER;
BEGIN
...
-- Step 12.2: Create delivery_agents table
CREATE TABLE IF NOT EXISTS delivery_agents (
  -- Primary Key
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  
  -- Authentication Link (will add foreign key constraint explicitly)
  user_id uuid UNIQUE,
  ...
  -- Document URLs (stored as image URLs after upload to Supabase Storage)
  selfie_url text,
...
-- Step 12.3: Add the CORRECT foreign key constraint pointing to auth.users
-- This is CRITICAL - must point to auth.users, NOT users table
-- We do a final drop to ensure no constraint exists, then add the correct one
DO $$
BEGIN
  -- Final drop to ensure no constraint exists
  BEGIN
    EXECUTE 'ALTER TABLE delivery_agents DROP CONSTRAINT IF EXISTS delivery_agents_user_id_fkey CASCADE';
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignore errors - constraint might not exist
  END;
  
  -- Check if constraint already exists to avoid duplicate_object error
  IF NOT EXISTS (
  ...
  ELSE
    ...
    -- Verify existing constraint points to auth.users
    IF EXISTS (
      SELECT 1 
      FROM information_schema.table_constraints AS tc
      JOIN information_schema.key_column_usage AS kcu
      ...
      RAISE NOTICE '✅✅✅ Existing constraint is CORRECT - points to auth.users';
    ELSE
      RAISE NOTICE '⚠️ Existing constraint is WRONG - dropping and recreating...';
      EXECUTE 'ALTER TABLE delivery_agents DROP CONSTRAINT IF EXISTS delivery_agents_user_id_fkey CASCADE';
      EXECUTE 'ALTER TABLE delivery_agents ADD CONSTRAINT delivery_agents_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE';
      RAISE NOTICE '✅✅✅ Recreated constraint correctly pointing to auth.users';
    END IF;
  END IF;
END $$;

-- Step 12.4: Create indexes for better query performance
...
-- Step 12.5: Create trigger function for updated_at
...
-- Step 12.6: Create trigger for updated_at
...
-- Step 12.7: Disable RLS (Row Level Security) for delivery_agents table
ALTER TABLE delivery_agents DISABLE ROW LEVEL SECURITY;

-- Step 12.8: Add comments for documentation
COMMENT ON TABLE delivery_agents IS 'Stores complete registration details for delivery agents/riders';
COMMENT ON COLUMN delivery_agents.user_id IS 'Links to Supabase auth.users(id) for authentication - CRITICAL: Points to auth.users, NOT users table';
COMMENT ON COLUMN delivery_agents.selfie_url IS 'URL of the selfie photo taken during registration (stored in Supabase Storage)';
...
-- Step 12.9: Verify the foreign key constraint is correct
SELECT 
  '✅ Step 12.9: Foreign Key Constraint Verification' as status,
  tc.constraint_name,
  kcu.column_name,
  ccu.table_schema || '.' || ccu.table_name as points_to,
  CASE 
    WHEN ccu.table_schema = 'auth' AND ccu.table_name = 'users' THEN '✅✅✅ CORRECT - Points to auth.users'
    ELSE '❌ WRONG - Points to ' || ccu.table_schema || '.' || ccu.table_name || ' - MUST BE FIXED!'
...
-- Verify delivery_agents foreign key constraint
SELECT 
  '✅✅✅ VERIFICATION: Delivery Agents Foreign Key Constraint' as check_type,
  tc.constraint_name,
  ccu.table_schema || '.' || ccu.table_name as points_to,
  CASE 
    WHEN ccu.table_schema = 'auth' AND ccu.table_name = 'users' THEN '✅✅✅ CORRECT - Points to auth.users'
...
✅✅✅ CORRECTLY configured foreign key constraint to point to auth.users(id) - NOT users table!
✅✅✅ This fixes the error: "Key (user_id)=(...) is not present in table 'users'" - registration will now work!
...
✅✅✅ Delivery agents foreign key constraint CORRECTLY points to auth.users - registration will work!
✅✅✅ The error "Key (user_id)=(...) is not present in table 'users'" will NOT occur anymore!
...
10. ✅✅✅ The foreign key constraint is CORRECTLY configured - registration will work without errors!
...
- ✅✅✅ CRITICAL: delivery_agents.user_id foreign key CORRECTLY references auth.users(id) - NOT users table!
- ✅✅✅ This ensures registration will work correctly - no foreign key constraint errors!
