...
-- 6. FIXES vendor shop user_id to match vendor auth user_id (ORDERS WILL WORK!)
-- 
...
-- ========================================
-- PART 8: VENDOR ORDERS DIAGNOSTICS
-- ========================================
-- This section helps diagnose why orders aren't showing in vendor app
-- ========================================

-- Step 8.1: Check shop and its user_id
SELECT 
  '🔍 VENDOR ORDERS DIAGNOSTIC: Shop Information' as diagnostic_step,
  id as shop_id,
  name as shop_name,
...
-- Step 8.2: Check if shop's user_id exists in users table
SELECT 
  '🔍 VENDOR ORDERS DIAGNOSTIC: Shop User ID Validation' as diagnostic_step,
  s.id as shop_id,
  s.name as shop_name,
  s.user_id as shop_user_id,
  CASE 
    WHEN u.id IS NOT NULL THEN '✅ User exists - Shop user_id is valid'
    ELSE '❌ User NOT found - This is the problem! Shop user_id does not match any user'
  END as user_status,
...
-- Step 8.3: Check orders for this shop
SELECT 
  '🔍 VENDOR ORDERS DIAGNOSTIC: Orders for Shop' as diagnostic_step,
...
-- Step 8.4: Show recent orders for this shop
SELECT 
  '🔍 VENDOR ORDERS DIAGNOSTIC: Recent Orders' as diagnostic_step,
  id as order_id,
  order_number,
  shop_id,
...
-- Step 8.5: Show all shops and their user_ids for comparison
SELECT 
  '🔍 VENDOR ORDERS DIAGNOSTIC: All Shops' as diagnostic_step,
  id as shop_id,
  name as shop_name,
  user_id,
  email,
...
-- Step 8.6: Show all users (to find the vendor's user_id)
SELECT 
  '🔍 VENDOR ORDERS DIAGNOSTIC: All Users' as diagnostic_step,
  id as user_id,
  email,
  phone,
  name
...
-- ========================================
-- PART 9: FIX SHOP USER_ID (AUTOMATIC FIX)
-- ========================================
-- This section automatically fixes the shop user_id to match vendor auth user_id
-- Vendor User ID from logs: 29353d7e-e658-4c43-9921-1cd9ebe1dcbf
-- Shop ID: shop-1766319629349-etq87nd4v
-- ========================================

-- Step 9.1: Check vendor auth user exists
SELECT 
  '🔍 STEP 9.1: Vendor Auth User Check' as step,
  id as auth_user_id,
  email,
  phone,
  created_at
...
-- Step 9.2: Check shop before fix
SELECT 
  '🔍 STEP 9.2: Shop Before Fix' as step,
  id as shop_id,
  name as shop_name,
...
-- Step 9.3: Update shop user_id to match vendor auth user_id
UPDATE shops 
SET user_id = '29353d7e-e658-4c43-9921-1cd9ebe1dcbf'
WHERE id = 'shop-1766319629349-etq87nd4v';

-- Step 9.4: Verify the fix
SELECT 
  '✅ STEP 9.4: Shop User ID Fix Verification' as step,
  s.id as shop_id,
  s.name as shop_name,
  s.user_id as shop_user_id,
  ...
  CASE 
    ...
    ELSE '❌ STILL BROKEN - Check the user_id values'
  END as fix_status
FROM shops s
LEFT JOIN auth.users au ON s.user_id = au.id
WHERE s.id = 'shop-1766319629349-etq87nd4v';

-- Step 9.5: Verify orders will be found
SELECT 
  '✅ STEP 9.5: Orders Verification' as step,
  COUNT(*) as total_orders_for_shop,
  shop_id
FROM orders
WHERE shop_id = 'shop-1766319629349-etq87nd4v'
GROUP BY shop_id;

SELECT '✅ Step 9: Shop user_id automatically fixed to match vendor auth user_id' as status;
...
✅ Added vendor orders diagnostics
✅ Fixed shop user_id to match vendor auth user_id (vendor orders will now work!)
...
✅ Shop user_id fixed - vendor orders should appear in vendor app now!
...
5. If orders not showing, follow the fix instructions above
...
- If vendor orders are not displaying, shop.user_id must correspond to vendor logged-in user_id
